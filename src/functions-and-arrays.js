...
// Iteration #3.2 Bonus:
const things = [6, 12, "miami", 1, true, "barca", "200", "lisboa", 8, 10];
...
function sum(things) {
  ...
  for (i = 0; i < things.length; i++) {
    if (typeof things[i] === "number") {
      suma = suma + things[i];
    } else if (typeof things[i] === "string") {
      suma = suma + things[i].length;
    } else if (typeof things[i] === "boolean") {
      if (things[i] === true) {
        suma = suma + 1;
      } else {
        suma = suma + 0;
      ...
      throw new Error(
        "There is a value that is not a number, string or boolean"
      );
    }
  }
  return suma;
